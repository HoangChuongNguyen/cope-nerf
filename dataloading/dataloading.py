...
def get_dataloader(cfg, mode='train',
                   shuffle=True, n_views=None, seed=None):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, num_workers=n_workers, 
        shuffle=shuffle, pin_memory=True, worker_init_fn=get_worker_init_fn(seed)
    )
...
def get_data_fields(cfg, mode='train'):
    ...
    fields = {}
    load_ref_img = (cfg['training']['flow_rgb_weight']!=0.0)
    ...
    if dataset_name=='any':
        img_field = DataField( 
                ...
                # use_DPT=use_DPT, 
                mode=mode,spherify=cfg['dataloading']['spherify'], 
                load_ref_img=load_ref_img, 
                # customized_poses=cfg['dataloading']['customized_poses'],
                # customized_focal=cfg['dataloading']['customized_focal'],
                resize_factor=cfg['dataloading']['resize_factor'], 
                # depth_net=cfg['dataloading']['depth_net'], 
                crop_size=cfg['dataloading']['crop_size'], 
                # random_ref=cfg['dataloading']['random_ref'], 
                random_ref_interval=cfg['dataloading']['random_ref_interval'], 
                # norm_depth=cfg['dataloading']['norm_depth'],
                load_gt_depth=cfg['dataloading']['load_gt_depth'],
                load_colmap_poses=cfg['dataloading']['load_colmap_poses'], sample_rate=cfg['dataloading']['sample_rate'],
                load_flow=cfg['dataloading']['load_flow'], resolution=cfg["training"]["resolution"])
    else:
