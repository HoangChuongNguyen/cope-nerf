...
def load_flows(image_list, datadir, random_ref_interval, H=None, W=None, crop_ratio=1):
    # if crop_ratio != 1:
    #     raise NotImplementedError('Not implement cropping the optical flow -> Need to process this properly')
    flow_fw_list = []
    flow_bw_list = []
    
    for ref_interval in random_ref_interval:
        interval_flow_fw_list = []
        interval_flow_bw_list = []
        for image_name in image_list:
            frame_id = image_name.split('.')[0]
            flow_fw_path = os.path.join(f'{datadir}/flow_fw_{ref_interval}rate/{frame_id}.npy')
            flow_bw_path = os.path.join(f'{datadir}/flow_bw_{ref_interval}rate/{frame_id}.npy')
            flow_fw = np.load(flow_fw_path) if os.path.isfile(flow_fw_path) else (np.zeros((2,H,W))+10e5)
            flow_bw = np.load(flow_bw_path) if os.path.isfile(flow_bw_path) else (np.zeros((2,H,W))+10e5)
            interval_flow_fw_list.append(flow_fw)
            interval_flow_bw_list.append(flow_bw)
        # if np.mean(interval_flow_fw_list) == 10e5: assert False, 'Invalid optical flow, all zeros'
        flow_fw_list.append(interval_flow_fw_list)
        flow_bw_list.append(interval_flow_bw_list)
    return flow_fw_list, flow_bw_list
